...
KNOWN_THIRD_PARTY_OWNER_PREFIXES = {
    "android.support",
    "androidx",
    "com.android",
    "com.facebook",
...
KNOWN_PACKAGE_CLASSIFICATIONS = [
    ...
    ("androidx.media3", "Android", "androidx.media3::media3"),
...
def has_known_third_party_prefix(package_name):
    package_name = normalize_pkg(package_name)
...
def is_developer_written(artifact_pkg, app_package_name):
    ...
    for artifact_token, app_token in zip(artifact_tokens, app_tokens):
        if artifact_token != app_token:
            break
        common_tokens.append(artifact_token)

    if len(common_tokens) < 2:
        return False

    common_prefix = ".".join(common_tokens)
    if has_known_third_party_prefix(common_prefix):
        return False

    owner_token = common_tokens[1]
    informative_shared_tokens = [
        token
        for token in common_tokens[2:]
        if token not in GENERIC_PACKAGE_PREFIXES and len(token) >= 3
    ]

    if (
        owner_token in GENERIC_PACKAGE_PREFIXES
        or len(owner_token) < 3
    ) and not informative_shared_tokens:
        return False
...
def classify_with_libscout(artifact_pkg, app_package_name, version_code, lib_category_map):
    ...
    for root_package, category, lib_name in entries:
        if is_incompatible_libscout_match(artifact_pkg, category, lib_name):
            continue
...
def is_incompatible_libscout_match(artifact_pkg, category, lib_name):
    artifact_pkg = normalize_pkg(artifact_pkg)
    category = "" if category is None else str(category).strip().lower()
    lib_name = "" if lib_name is None else str(lib_name).strip().lower()
    ...
    if packages_match(artifact_pkg, "org.bouncycastle") and (
        lib_name.startswith("facebook")
        or category in {"socialmedia", "android", "advertising", "analytics", "cloud"}
    ):
        return True

    if packages_match(artifact_pkg, "com.google.android.gms") and category != "android":
        return True

    if packages_match(artifact_pkg, "com.google.firebase") and category != "android":
        return True

    if packages_match(artifact_pkg, "androidx") and category == "socialmedia":
        return True
