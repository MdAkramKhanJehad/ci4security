...
def candidate_prefixes(dot_path: str):
    ...
    while cur and cur not in seen:
        tokens = cur.split(".")
        if (len(cur) >= 3 and cur not in {"com", "org", "net", "io"} and all(len(tok) >= 2 for tok in tokens)):
        ...
        if stripped != last:
            ...
            if (len(alt) >= 3 and alt not in {"com", "org", "net", "io"} and all(len(tok) >= 2 for tok in alt.split("."))):
    ...
    final_candidates = [c for c in candidates if len(c) >= 3 and c not in {"com", "org", "net", "io"} and all(len(tok) >= 2 for tok in c.split("."))]
...
def match_against_libprofiles(candidates, lib_map):
    for c in candidates:
        ...
        for lib_name, category in lib_map.items():
            lib_norm = re.sub(r"[.\-_:]", "", lib_name.lower())

            if (c_norm == lib_norm or c_strip == lib_norm or lib_norm.startswith(c_norm) or lib_norm.startswith(c_strip) or c_norm.startswith(lib_norm) or c_strip.startswith(lib_norm)):
...
def match_in_pkgonly(artifact: str, pkg_only, lib_map: dict):
    ...
    for lib_name, root in items:
        ...
        for c in cands:
            if c == root or c.startswith(root + ".") or root.startswith(c + "."):
...
def match_in_libmatches(artifact: str, lib_matches, lib_map: dict):
    ...
    for lib in lib_matches:
        ...
        lib_name = lib.get("libName", "").strip()
        if not lib_name:
            continue

        root_prefix = lib_name.split("::")[0].strip()
        root = normalize_pkg(root_prefix)

        if any(root.startswith(x) for x in ["androidx", "com.android", "org.jetbrains"]):
            continue

        for c in cands:
            if c.startswith(root) or c == root:
                cat = find_category_for_lib(lib_name, lib_map)
                return (cat or "others"), lib_name
        ...
        if json_path:
            try:
                with open(json_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                code_location, lib_name = match_in_pkgonly(artifact_dir, data.get("lib_packageOnlyMatches"), lib_map)
                if not code_location:
                    code_location, lib_name = match_in_libmatches(artifact_dir, data.get("lib_matches"), lib_map)
